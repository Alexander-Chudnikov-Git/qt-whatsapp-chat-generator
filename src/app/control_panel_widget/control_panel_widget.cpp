...
void ControlPanelWidget::setupUi()
{
	...
	m_hide_header_checkbox->setChecked(
		UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_CHAT_HIDE_HEADER).toBool());
	m_hide_footer_checkbox->setChecked(
		UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_CHAT_HIDE_FOOTER).toBool());
	m_hide_dark_theme_checkbox->setChecked(
		UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_CHAT_HIDE_DARK_THEME).toBool());
	m_hide_iphone_checkbox->setChecked(
		UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_CHAT_HIDE_IPHONE).toBool());
	m_hide_percentage_checkbox->setChecked(
		UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_CHAT_HIDE_PERCENTAGE).toBool());
	m_hide_payment_checkbox->setChecked(
		UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_CHAT_HIDE_PAYMENT).toBool());
	m_hide_dual_sim_checkbox->setChecked(
		UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_CHAT_HIDE_DUAL_SIM).toBool());

	// Other Input Widgets
	m_network_combobox = new QComboBox();
	QStringList networks =
		UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_CHAT_NETWORKS).toStringList();
	m_network_combobox->addItems(networks);
	m_network_combobox->setCurrentText(
		UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_CHAT_NETWORK).toString());

	m_time_edit = new QTimeEdit();
	m_time_edit->setTime(
		UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_CHAT_TIME).toTime());

	m_battery_spinbox = new QSpinBox();
	m_battery_spinbox->setRange(0, 100);
	m_battery_spinbox->setSuffix("%");
	m_battery_spinbox->setValue(
		UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_CHAT_BATTERY).toInt());

	m_name_line_edit = new QLineEdit();
	m_name_line_edit->setText(
		UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_CHAT_NAME).toString());
	...
	m_directory_selector_line_edit->setText(
		UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_IMAGE_PATH).toString());
...
void ControlPanelWidget::setupConnections()
{
	...
	connect(this->m_chat_parser, &UTILS::WhatsappChatParser::parsingStatus, this, [this](const QString &message) {
		emit parsingStatus(message);
	});
	connect(this->m_chat_generator, &UTILS::WhatsappChatGenerator::generatorStatus, this, [this](const QString &message) {
		emit parsingStatus(message);
	});
...
{
	auto default_path =
		UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_CHAT_PATH).toString();

	QString file_path = QFileDialog::getOpenFileName(this, tr("select_input_file_lable"), default_path,
													 tr("xlsx_file_selector (*.xls *.xlsx)"));
...
void ControlPanelWidget::onDirectoryBrowseClicked()
{
	...
	QString directory_path = QFileDialog::getExistingDirectory(this, tr("select_directory_label"), default_path,
...
void ControlPanelWidget::onRunClicked()
{
	...
	{
		m_run_button->setEnabled(
			false); // Consider if disabling the button is the desired behavior. Maybe just inform the user.
	...
	error_connection = connect(
		this->m_chat_generator, &UTILS::WhatsappChatGenerator::generatorError, this,
		[this, done_connection]() {
			disconnect(done_connection);
			setEnabled(true);
		},
		Qt::SingleShotConnection);

	done_connection = connect(
		this->m_chat_generator, &UTILS::WhatsappChatGenerator::generatorDone, this,
		[this, error_connection]() {
			...
			auto save_path =
				UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_IMAGE_PATH).toString();
			auto filename_prefix =
				UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_IMAGE_PREFIX).toString();
			auto file_path =
				UTILS::SettingsManager::instance()->getValue(UTILS::SettingsManager::Setting::FAKE_CHAT_PATH).toString();
			QFileInfo file_info(file_path);
