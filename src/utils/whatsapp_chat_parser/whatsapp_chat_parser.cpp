...
namespace UTILS
{
WhatsappChatParser::WhatsappChatParser(QObject* parent) : QObject(parent)
{}
...
bool WhatsappChatParser::parseRow(std::shared_ptr<QXlsx::Cell> cell_inbound,
								  std::shared_ptr<QXlsx::Cell> cell_outbound,
								  std::shared_ptr<QXlsx::Cell> cell_date,
								  int						   row)
{
...
QDateTime WhatsappChatParser::readDateTime(std::shared_ptr<QXlsx::Cell> cell, int row)
{
	...
	if (cell)
	{
		...
		else
		{
			...
			QStringList date_time_formats = {"dd.MM.yyyy", "dd-MM-yyyy", "dd/MM/yyyy", "yyyy.MM.dd",
											 "yyyy-MM-dd", "yyyy/MM/dd", "MM.dd.yyyy", "MM-dd-yyyy",
											 "MM/dd/yyyy", "yyyy.dd.MM", "yyyy-dd-MM", "yyyy/dd/MM"};
...
QString WhatsappChatParser::addMessage(WhatsappMessage::Type type,
									   const QString&		 text,
