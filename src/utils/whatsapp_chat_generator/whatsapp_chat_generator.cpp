	...
	connect(m_web_page, &QWebEnginePage::loadFinished, this, [this](bool success) {
		QTimer::singleShot(500, this, [this, success]() {
			if (success)
			{
				...
				auto website_ads =
					SettingsManager::instance()->getValue(SettingsManager::Setting::FAKE_CHAT_ADS).toStringList();
				...
				QTimer::singleShot(3000, this, [this]() {
					...
					setNetworkString(
						SettingsManager::instance()->getValue(SettingsManager::Setting::FAKE_CHAT_NETWORK).toString());
					setTimeString(SettingsManager::instance()
									  ->getValue(SettingsManager::Setting::FAKE_CHAT_TIME)
									  .toTime()
									  .toString("H:mm"));
					setPowerString(
						SettingsManager::instance()->getValue(SettingsManager::Setting::FAKE_CHAT_BATTERY).toString());
...
void WhatsappChatGenerator::processNextMessage()
{
	...
	createMessage(msg);
	QTimer::singleShot(SettingsManager::instance()->getValue(SettingsManager::Setting::FAKE_CHAT_DELAY).toInt(), this,
					   &WhatsappChatGenerator::processNextMessage);
}
...
void WhatsappChatGenerator::takeScreenshot()
{
	...
	this->m_web_page->runJavaScript(jsCode, [this](const QVariant& result) {
		if (!result.isValid())
		{
		...
		if (result.typeId() == QMetaType::QVariantMap)
		{
			...
			if (!resultMap.contains("x") || !resultMap.contains("y") || !resultMap.contains("width") ||
				!resultMap.contains("height"))
			{
			...
			QTimer::singleShot(SettingsManager::instance()->getValue(SettingsManager::Setting::FAKE_SCREENSHOT_DELAY).toInt(),
							   this, &WhatsappChatGenerator::checkScrollPosition);
		}
		else
		{
	...
	this->m_web_page->runJavaScript(jsCode, [this](const QVariant& result) {
		...
		if (result.typeId() == QMetaType::QVariantMap)
		{
			...
			if (resultMap.contains("scrolled") && resultMap["scrolled"].toBool())
			{
				...
				QTimer::singleShot(
					SettingsManager::instance()->getValue(SettingsManager::Setting::FAKE_SCREENSHOT_DELAY).toInt(), this,
					&WhatsappChatGenerator::takeScreenshot);
			}
			else
			{
	...
	QString script = QString(R"(
        (function()
        {
            ...
            $(".close-btn").click();
						 ...
						 .arg(message.getText(), message.getDateTime().toString("HH:mm"), "", date,
							  (message.getType() == WhatsappMessage::Type::Inbound) ? "Receive" : "Sent");
